...
export const en: Dict = {
  ...
  launcher: {
    eyebrow: 'desktop app · tauri 2 + rust',
    titleA: 'DSH',
    titleB: 'Launcher',
    sub: 'A multi-version, multi-instance desktop launcher for DeepSeek Harness',
    lead: 'Install multiple @deepseek-ai/dsh versions into isolated directories, give every instance its own DSH_HOME, Profile and environment variables, then launch in one click — the Web GUI opens in its own window. A built-in plugin market, system tray and autostart complete the package.',
    version: 'v0.2.0',
    license: 'MIT',
    platforms: 'Windows · macOS · Linux',
    stack: 'Tauri 2 · Vue 3 · TypeScript · Rust · Arco Design Vue',
    ctaDownload: 'Download',
    ctaRepo: 'GitHub Repo',
    shotAlt: 'DSH Launcher main window screenshot',
    back: 'Back to home',
    feat: {
      eyebrow: 'features',
      title: 'One launcher for every DSH',
      lead: 'From installing versions to launching instances and adding plugins — daily DSH ops live in a single desktop window.',
      items: [
        { icon: 'stack', title: 'Multi-version installs', desc: 'Query the npm registry and install multiple @deepseek-ai/dsh versions into isolated directories — no interference.' },
        { icon: 'box', title: 'Multi-instance management', desc: 'Create several instances per version, each with its own name, Profile and runtime environment variables.' },
        { icon: 'zap', title: 'One-click launch', desc: 'Pick an instance and a Profile, hit launch; the dsh web URL is parsed and the Web GUI opens in a dedicated webview window.' },
        { icon: 'edit', title: 'Env overrides', desc: 'Add or remove runtime env vars per instance, injected at spawn; DSH_HOME itself is injected by the launcher per the chosen mode.' },
        { icon: 'plug', title: 'Plugin market', desc: 'Browse plugins published on dsh-plug.in, search by name or description, and install to any instance Profile via a wizard.' },
        { icon: 'list', title: 'Per-Profile plugin control', desc: 'Filter plugins by Profile, toggle individually or in bulk; core @deepseek-ai/* packages stay hidden.' },
        { icon: 'layout', title: 'System tray', desc: 'Double-click to jump into a running instance, right-click to open/stop each Profile; quitting the launcher stops all instances.' },
        { icon: 'check', title: 'Autostart', desc: 'A real autostart registration via the Tauri plugin, plus close-to-tray so the launcher stays out of the way.' },
      ],
    },
    homes: {
      eyebrow: 'dsh_home',
      title: 'Three DSH_HOME modes, isolated as needed',
      lead: 'Every instance picks where its data lives — from fully shared to fully isolated.',
      modes: [
        { icon: 'swap', title: 'Reuse existing', desc: 'Point at an existing DSH_HOME so multiple instances share one configuration and data set.' },
        { icon: 'box', title: 'Adopt the default', desc: 'Automatically adopt the user default ~/.dsh and let the launcher take over the environment you already use.' },
        { icon: 'shield', title: 'Dedicated isolation', desc: 'Create a fresh, dedicated DSH_HOME under the data directory, registered automatically — zero cross-contamination.' },
      ],
    },
    market: {
      eyebrow: 'plugin market',
      title: 'Built-in plugin market, three channels',
      lead: 'Straight from the dsh-plug.in catalog into a Profile — one wizard, end to end.',
      channels: [
        { badge: 'R', name: 'Stable', desc: 'GitHub releases / npm latest', tone: 'emerald' },
        { badge: 'B', name: 'Beta', desc: 'pre-releases / npm next', tone: 'amber' },
        { badge: 'A', name: 'Latest commit', desc: 'Newest GitHub commit', tone: 'danger' },
      ],
      steps: ['Pick a plugin', 'Pick a channel & version', 'Pick an instance & Profile', 'Create the task & install'],
      note: 'Dependency buildScripts are allowed automatically and the plugin is registered in the Profile package.json; non-bundle plugins also get a cordis.patch.yml insert row.',
    },
    download: {
      eyebrow: 'download',
      title: 'Download, or build from source',
      lead: 'Prebuilt installers ship on GitHub Releases; the repo carries full CI, and a single local command packages it too.',
      ctaReleases: 'Go to Releases',
      ctaSource: 'View source',
      buildTitle: 'Build from source',
      buildNote: 'Prerequisites: Node ≥ 20, pnpm, Rust stable, WebView2.',
      dataNote: 'Runtime data lives in %APPDATA%\\in.dsh-plug.dsh-launcher\\ — config, isolated versions, dedicated DSH_HOMEs and instance logs.',
    },
  },
