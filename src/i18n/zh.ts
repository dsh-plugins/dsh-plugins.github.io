...
export const zh = {
  ...
  launcher: {
    ...
    titleA: 'DSH',
    titleB: 'Launcher',
    sub: '多版本、多实例的 DeepSeek Harness 桌面启动器',
    lead: '把多个 @deepseek-ai/dsh 版本装进隔离目录，为每个实例配上独立的 DSH_HOME、Profile 与环境变量；一键启动后自动在独立窗口打开 Web GUI。插件市场、系统托盘、开机自启，一应俱全。',
    version: 'v0.2.0',
    license: 'MIT',
    ...
    ctaDownload: '下载安装包',
    ctaRepo: 'GitHub 仓库',
    shotAlt: 'DSH Launcher 主界面截图',
    back: '返回主页',
    feat: {
      eyebrow: 'features',
      title: '一个启动器，装下所有 DSH',
      lead: '从安装版本到启动实例、再到装插件——DSH 的日常运维全部收进一个桌面窗口。',
      items: [
        { icon: 'stack', title: '多版本安装', desc: '从 npm registry 查询并安装多个 @deepseek-ai/dsh 版本到隔离目录，互不干扰。' },
        { icon: 'box', title: '多实例管理', desc: '同一版本可创建多个实例，各自拥有独立的名称、Profile 与运行时环境变量。' },
        { icon: 'zap', title: '一键启动', desc: '选择实例与 Profile 一键启动；自动解析 dsh web 输出的 URL，在独立 Webview 窗口打开 Web GUI。' },
        { icon: 'edit', title: '环境变量复写', desc: '实例设置页增删运行时环境变量，启动时注入子进程；DSH_HOME 由启动器按所选模式注入。' },
        { icon: 'plug', title: '插件市场', desc: '浏览 dsh-plug.in 发布的插件，按名称与描述搜索，向导式安装到任意实例的 Profile。' },
        { icon: 'list', title: '按 Profile 管理插件', desc: '筛选 Profile 查看其下插件，单个或多选批量启用/禁用；核心 @deepseek-ai/* 包自动隐藏。' },
        { icon: 'layout', title: '系统托盘', desc: '双击直达运行中实例，右键菜单打开/停止各 Profile；退出启动器自动终止所有实例进程。' },
        { icon: 'check', title: '开机自启', desc: '设置页一键开关，经 autostart 插件真正注册；关闭窗口可最小化到托盘。' },
      ],
    },
    homes: {
      eyebrow: 'dsh_home',
      title: '三种 DSH_HOME 模式，按需隔离',
      lead: '每个实例都能选择自己的数据归宿——从完全共享到完全隔离。',
      modes: [
        { icon: 'swap', title: '复用已有', desc: '指向一个现成的 DSH_HOME，多个实例共享同一份配置与数据。' },
        { icon: 'box', title: '收录默认', desc: '自动收录用户默认的 ~/.dsh，让启动器接管你已经在用的环境。' },
        { icon: 'shield', title: '专属隔离', desc: '为实例新建专属 DSH_HOME，自动在数据目录创建并注册，互不污染。' },
      ],
    },
    market: {
      eyebrow: 'plugin market',
      title: '内置插件市场，三个版本渠道',
      lead: '直连 dsh-plug.in 市场目录，从选插件到装进 Profile 只要一个向导。',
      channels: [
        { badge: 'R', name: '稳定版', desc: 'GitHub releases / npm latest', tone: 'emerald' },
        { badge: 'B', name: '测试版', desc: 'pre-releases / npm next', tone: 'amber' },
        { badge: 'A', name: '最新提交', desc: 'GitHub 最新 commit', tone: 'danger' },
      ],
      steps: ['选择插件', '选择渠道与版本', '选择实例与 Profile', '创建任务并安装'],
      note: '自动允许依赖的 buildScripts，并在 Profile 的 package.json 注册插件；非 bundle 插件额外写入 cordis.patch.yml insert 行。',
    },
    download: {
      eyebrow: 'download',
      title: '下载，或从源码构建',
      lead: '预编译安装包发布在 GitHub Releases；仓库自带完整 CI，本地也可以一条命令打包。',
      ctaReleases: '前往 Releases',
      ctaSource: '查看源码',
      buildTitle: '从源码构建',
      buildNote: '前置：Node ≥ 20、pnpm、Rust stable、WebView2。',
      dataNote: '运行数据位于 %APPDATA%\\in.dsh-plug.dsh-launcher\\ —— 配置、隔离版本、专属 DSH_HOME 与实例日志都在其中。',
    },
  },
